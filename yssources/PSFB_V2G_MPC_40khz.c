...
void main()
{
   ...
   EALLOW;
   PieVectTable.EPWM3_INT = &epwm3_timer_isr;  // ePWM1中断入口
...
interrupt void epwm3_timer_isr(void)
{
	...
	//Ug = LPfilter(mSample[1] * HallRatioVg, Ug, 5000, MPC_Ts);  // 采样方向和正方向相同
	...
	EPwm3Regs.CMPA.half.CMPA = S4_min * MPCperiod;  // 寄存器设置

	EPwm4Regs.CMPA.half.CMPA = S1_min * MPCperiod;  // 寄存器设置
	EPwm5Regs.CMPA.half.CMPA = S2_min * MPCperiod;  // 寄存器设置
	EPwm6Regs.CMPA.half.CMPA = S3_min * MPCperiod;  // 寄存器设置
